...
class Gestor_de_cookies{
	...
	public static function set($id_usuario,$cookie_real=true){
		...
		$useris=new Useris();
		$useris->set_id_usuario($id_usuario);
		$useris->set_fecha(date('Y-m-d h:i:s'));
		$useris->set_sesion($sesion); #WTF
		$useris->set_machine($_SERVER['REMOTE_ADDR']);
		$useris->set_id_usuariocode(1); # 2 Si es un usuario externo. 1 Si es Interno.
	...
	public static function get($value){
		...
		$recordset=Useris::select(array('cstr'=>$value,'id_usuariocode'=>1));
//                var_dump($recordset);
