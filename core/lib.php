...
function developer_log($string)
{
	...
	if(DEVELOPER AND ACTIVAR_LOG_APACHE) {
		...
		error_log($nombre.$string);
	}
	if(DEVELOPER AND $GLOBALS['ACTIVAR_LOG_NAVEGADOR']) echo "<div class='debug'>".$string."</div>";
	if(DEVELOPER AND ACTIVAR_LOG_CONSOLA_NAVEGADOR) return log_consola_navegador($string);
}
...
{
	return ereg("^[a-zA-Z]{2}[0-9]{6}$", $mercalpha);
}
